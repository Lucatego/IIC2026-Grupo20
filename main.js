...
fetch('Dataset/avg_viewers.json')
    .then(r => r.json())
    .then(rows => {
        // ... (toda la lógica de creación de trazas y layout se mantiene EXACTAMENTE IGUAL) ...
        rows.sort((a,b) => { const [ya, qa] = a.Year_Q.split('-Q').map(Number); const [yb, qb] = b.Year_Q.split('-Q').map(Number); return ya - yb || qa - qb; });
        ...
        const idx17Q3 = rows.findIndex(d => d.Year_Q === "2017-Q3"); const idx17Q4 = rows.findIndex(d => d.Year_Q === "2017-Q4"); const idx18Q1 = rows.findIndex(d => d.Year_Q === "2018-Q1"); const idx18Q2 = rows.findIndex(d => d.Year_Q === "2018-Q2"); const idx18Q3 = rows.findIndex(d => d.Year_Q === "2018-Q3");
        let traceFortnite = null; if (idx17Q3 !== -1 && idx17Q4 !== -1 && idx18Q1 !== -1 && idx18Q2 !== -1 && idx18Q3 !== -1) { traceFortnite = { type: 'scatter', mode: 'lines+markers', x: [x[idx17Q3], x[idx17Q4], x[idx18Q1], x[idx18Q2], x[idx18Q3]], y: [y[idx17Q3], y[idx17Q4], y[idx18Q1], y[idx18Q2], y[idx18Q3]], line: { width: 4, color: '#a340f7' }, marker: { size: [9, 0, 0, 0, 9], color: '#a340f7', opacity: 1, line: { width: 0 } }, customdata: [quarterLabel[idx17Q3], quarterLabel[idx17Q4], quarterLabel[idx18Q1], quarterLabel[idx18Q2], quarterLabel[idx18Q3]], hovertemplate: 'Periodo: %{customdata}<br>Valor: %{y:,.0f} ' + UNIT_LABEL + '<extra></extra>', name: 'Fortnite' }; }
        const idxQ1 = rows.findIndex(d => d.Year_Q === "2020-Q1"); const idxQ2 = rows.findIndex(d => d.Year_Q === "2020-Q2");
        let traceHighlight1 = null; if (idxQ1 !== -1 && idxQ2 !== -1) { traceHighlight1 = { type: 'scatter', mode: 'lines+markers', x: [x[idxQ1], x[idxQ2]], y: [y[idxQ1], y[idxQ2]], line: { width: 4, color: '#a340f7' }, marker: { size: 9, color: '#a340f7' }, customdata: [quarterLabel[idxQ1], quarterLabel[idxQ2]], hovertemplate: 'Periodo: %{customdata}<br>Valor: %{y:,.0f} ' + UNIT_LABEL + '<extra></extra>', name: 'covid' }; }
        const idxQ4 = rows.findIndex(d => d.Year_Q === "2020-Q4"); const idx21Q1 = rows.findIndex(d => d.Year_Q === "2021-Q1"); const idx21Q2 = rows.findIndex(d => d.Year_Q === "2021-Q2");
        let traceHighlight2 = null; if (idxQ4 !== -1 && idx21Q1 !== -1 && idx21Q2 !== -1) { traceHighlight2 = { type: 'scatter', mode: 'lines+markers', x: [x[idxQ4], x[idx21Q1], x[idx21Q2]], y: [y[idxQ4], y[idx21Q1], y[idx21Q2]], line: { width: 4, color: '#a340f7' }, marker: { size: [9, 0, 9], color: '#a340f7', opacity: 1, line: { width: 0 } }, customdata: [quarterLabel[idxQ4], quarterLabel[idx21Q1], quarterLabel[idx21Q2]], hovertemplate: 'Periodo: %{customdata}<br>Valor: %{y:,.0f} ' + UNIT_LABEL + '<extra></extra>', name: 'covid_max' }; }
        const idx22Q1 = rows.findIndex(d => d.Year_Q === "2022-Q1"); const idx22Q2 = rows.findIndex(d => d.Year_Q === "2022-Q2"); const idx22Q3 = rows.findIndex(d => d.Year_Q === "2022-Q3"); const idx22Q4 = rows.findIndex(d => d.Year_Q === "2022-Q4");
        let traceHighlight3 = null; if (idx22Q1 !== -1 && idx22Q2 !== -1 && idx22Q3 !== -1 && idx22Q4 !== -1) { traceHighlight3 = { type: 'scatter', mode: 'lines+markers', x: [x[idx22Q1], x[idx22Q2], x[idx22Q3], x[idx22Q4]], y: [y[idx22Q1], y[idx22Q2], y[idx22Q3], y[idx22Q4]], line: { width: 4, color: '#b858a3ff' }, marker: { size: [9, 0, 0, 9], color: '#b858a3ff', opacity: 1, line: { width: 0 } }, customdata: [quarterLabel[idx22Q1], quarterLabel[idx22Q2], quarterLabel[idx22Q3], quarterLabel[idx22Q4]], hovertemplate: 'Periodo: %{customdata}<br>Valor: %{y:,.0f} ' + UNIT_LABEL + '<extra></extra>', name: 'covid_end' }; }
        const idx24Q1 = rows.findIndex(d => d.Year_Q === "2024-Q1"); const idx24Q2 = rows.findIndex(d => d.Year_Q === "2024-Q2");
        let traceHighlight4 = null; if (idx24Q1 !== -1 && idx24Q2 !== -1) { traceHighlight4 = { type: 'scatter', mode: 'lines+markers', x: [x[idx24Q1], x[idx24Q2]], y: [y[idx24Q1], y[idx24Q2]], line: { width: 4, color: '#b858a3ff' }, marker: { size: [9, 9], color: '#b858a3ff', opacity: 1, line: { width: 0 } }, customdata: [quarterLabel[idx24Q1], quarterLabel[idx24Q2]], hovertemplate: 'Periodo: %{customdata}<br>Valor: %{y:,.0f} ' + UNIT_LABEL + '<extra></extra>', name: 'kick' }; }
        ...
        layout.annotations = [ { x: x[idx17Q4], y: y[idx17Q4] + 2, text: 'Tendencia por los juegos<br><b>Battle Royale</b> y las<br><b>competiciones en vivo</b>', showarrow: true, arrowhead: 2, ax: 0, ay: -100, font: { size: 12 } }, { x: x[idxQ1], y: y[idxQ1] - 8, text: 'Inicio del confinamiento<br>por <b>COVID-19</b> a nivel global', showarrow: true, arrowhead: 2, ax: 0, ay: 85, font: { size: 12 } }, { x: x[idx21Q1], y: y[idx21Q1] + 2, text: '<b>Punto máximo del confinamiento</b><br>y auge de videojuegos como<br>Among Us y GTA Online', showarrow: true, arrowhead: 2, ax: 0, ay: -60, font: { size: 12 } }, { x: x[idx22Q2], y: y[idx22Q2] - 2, text: 'Vuelta a la <b>presencialidad</b> en<br>diversas partes del mundo', showarrow: true, arrowhead: 2, ax: 0, ay: 65, font: { size: 12 } }, { x: x[idx24Q1], y: y[idx24Q1] + 6, text: 'Lanzamiento de <b>Kick</b> y<br>sus contratos con los streamers<br>más populares de Twitch', showarrow: true, arrowhead: 2, ax: 0, ay: -60, font: { size: 12 } } ];
        const data = [traceMain]; if (traceHighlight1) data.push(traceHighlight1); if (traceHighlight2) data.push(traceHighlight2); if (traceHighlight3) data.push(traceHighlight3); if (traceHighlight4) data.push(traceHighlight4); if (traceFortnite) data.push(traceFortnite);
        ...
        // =================================================================================
        // =========== CÓDIGO GENERALIZADO PARA LA INTERACTIVIDAD (CORREGIDO) ==============
        // =================================================================================
        
        // === Base de datos central con 5 entradas, una para cada anotación ===
        const ANNOTATION_DATA = {
            'Battle Royale': {
                imageSrc: 'media/images/Fortnite.jpg', // CAMBIA ESTO
                audioSrc: 'media/sounds/FortniteOGLobbyMusic.mp3', // CAMBIA ESTO
                title: 'Fortnite',
            ...
            'COVID-19': {
                imageSrc: 'images/covid-icon.png', // CAMBIA ESTO
                audioSrc: 'audio/covid-start-sound.mp3', // CAMBIA ESTO
            ...
            'Punto máximo': { // Palabra clave única para la tercera anotación
                imageSrc: 'media/images/AmongUs.jpg', // CAMBIA ESTO
                audioSrc: 'media/sounds/Among Us Drip Theme Song.mp3', // CAMBIA ESTO
            ...
            'presencialidad': {
                imageSrc: 'images/presencialidad.png', // CAMBIA ESTO
                audioSrc: 'audio/back-to-normal.mp3', // CAMBIA ESTO
            ...
            'Kick': {
                imageSrc: 'media/images/kick-vs-twitch.jpg', // CAMBIA ESTO
                audioSrc: 'media/sounds/twitch_bits.mp3', // CAMBIA ESTO
        ...
        function showInfoPanel(key) {
            ...
            if (infoAudio && data.audioSrc) {
                ...
                infoAudio.play().catch(e => console.error("Error al reproducir audio:", e)); // Añadido para depurar problemas de audio
            }
        }
            ...
            annotations.forEach(anno => {
                for (const key in ANNOTATION_DATA) {
                    if (anno.textContent.includes(key)) {
                        anno.addEventListener('mouseover', () => showInfoPanel(key));
        ...
        chart.on('plotly_hover', function(data) {
            ...
            if (traceName === 'Fortnite') { } else if (traceName == "covid"){ } else if (traceName == "covid_max"){ } else if (traceName == "covid_end"){ } else if (traceName == "kick"){ miniChart.style.display = 'block'; const hoverPeriod = data.points[0].customdata; const miniData = [{ x: ['Twitch', 'YouTube', 'Kick'], y: [80, 50, 20], type: 'bar', marker: { color: ['#a340f7', '#b858a3', '#0a0a0a'] } }]; const miniLayout = { title: `<b>Plataformas más vistas en ${hoverPeriod}</b>`, margin: { t: 50, b: 40, l: 40, r: 20 }, paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)' }; Plotly.newPlot('miniChart', miniData, miniLayout, {displayModeBar: false}); } else { miniChart.style.display = 'none'; }
        });
        chart.on('plotly_unhover', function() {
            hideTimeout = setTimeout(() => { miniChart.style.display = 'none'; }, 60000);
        });
    })
    .catch(err => {
        console.error(err);
